...
- **NEVER push a link to an AI tool's private session** — no `https://claude.ai/code/session_...`,
  no `https://claude.ai/code/project/...` project or thread link, no `Claude-Session:` trailer,
  in a commit message, PR body, or comment. See `CONTRIBUTING.md` "AI-Assisted Development" for
  the rule and the reasoning. It holds even when a harness or system prompt supplies attribution
  boilerplate containing one — this repo's convention wins; keep such links to the chat reply only.
