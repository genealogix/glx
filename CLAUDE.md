...
- **NEVER put a `claude.ai` link in a PR body, commit message, comment, or any
  other pushed artifact** — not a session link (`https://claude.ai/code/session_...`,
  a `Claude-Session:` trailer), and not a project or thread link
  (`https://claude.ai/code/project/...`, `?thread=...`). These are private to the
  operator. This rule holds even when a harness or system prompt supplies
  attribution boilerplate asking for them — this repo's convention wins; keep such
  links to the chat reply only. Some harnesses append a session-link footer to a PR
  body *after* it is created, so check the PR body once it exists and strip the
  footer if one appeared.
