...
- **GEDCOM/GEDZIP import now populates `media.properties.original_filename`** — The standard `original_filename` media property ("Original filename before import or renaming") was defined in the vocabulary but never populated by any import path. Relative `FILE` references — including media bundled in a GEDZIP archive, whose entries flow through the same conversion path — now record the source file's basename (pre-canonicalization, pre-rename) when the file is copied into `media/files/`. This preserves the actual source name in the case the property's own description exists for: a basename collision renaming the stored file (`photo.jpg` → `photo-2.jpg`), which previously lost the original name entirely. URL and absolute-path references are unaffected (their URI is preserved verbatim, so nothing is renamed), BLOB-derived files get no entry (their filename is synthesized at import), and an `original_filename` a source already carries is never overwritten. Purely additive for round-trips: GEDCOM export emits `uri` verbatim and maps no tag to the property. Adds `MediaPropertyOriginalFilename`/`MediaPropertyBlobSize` constants to `go-glx`. (#1121)
