...
- **Pre-commit `golangci-lint` hook now actually fires** — The `PreToolUse` hook added in #655 used `"matcher": "Bash(git commit*)"`, which Claude Code compiles as a JavaScript regex and tests against the **tool name** (`"Bash"`), not the command — so it never matched and the Claude-side lint pass was dead code (lefthook, #280, was the only effective gate). The matcher is now `"Bash"` (tool name, exact) with the command pattern moved to the per-handler `if: "Bash(git commit:*)"` field, which uses permission-rule syntax against the Bash subcommand, per the documented 2026 hooks schema. The inlined shell one-liner was extracted to `scripts/claude-hooks/pre-commit-golangci.sh` (`set -euo pipefail`, standalone-runnable, shellcheck-clean) so the logic is reviewable and testable, and it now resolves the project root via the documented `${CLAUDE_PROJECT_DIR}` rather than the undocumented `${CLAUDE_WORKING_DIRECTORY}`. The hook stays advisory (golangci-lint's exit 1 is non-blocking for `PreToolUse`); promoting it to a blocking gate is tracked separately in #870. Fixes #869.
