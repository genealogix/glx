...
- **Plugin discovery (Phase 1 of #95)** — `glx <name>` now executes `glx-<name>` from `PATH` when `<name>` is not a built-in command, forwarding the args **after** `<name>` plus stdio and propagating the child's exit code (git/kubectl plugin model; root flags appearing **before** `<name>` are dropped, matching `git -C dir foo bar`). `glx --plugins` lists every discovered plugin, annotating any whose name is shadowed by a built-in command. Built-ins always win; cobra's standard "unknown command" error is preserved for genuine typos. Phase 1 is foundational only — no archive data is passed to plugins yet. The JSON wire format and the validator/importer/exporter/hook protocols (and a Go SDK and registry) are deferred to follow-up phases of #95. (#95)
