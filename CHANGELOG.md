...
- **Release builds are reproducible again (`main.date` now commit-pinned)** — `.goreleaser.yml` embedded the build-invocation timestamp (`{{.Date}}`) into the binary via `-X main.date=...`, so two builds of the same tagged commit produced different bytes — defeating the reproducibility the build's other settings (`CGO_ENABLED=0`, `-trimpath`, `mod_timestamp: {{ .CommitTimestamp }}`) are engineered to provide, and failing any rebuild-and-compare supply-chain audit. The ldflag now uses `{{.CommitDate}}`, pinning the embedded date to the commit like the archive timestamps already were. `main.date` is display-only (`glx version`), so the only user-visible change is that the reported date is the release commit's date rather than the moment CI ran. Fixes #1049.
