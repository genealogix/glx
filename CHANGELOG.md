...
- **Go security tool versions are now pinned via a Go 1.24 `tool` directive** — Added a dedicated `tools/` module (`tools/go.mod`) whose `tool (...)` block pins `gosec` (`v2.22.4`) and `govulncheck` (`v1.3.0`), making it the single source of truth for both CI and local runs. The `security.yml` `gosec` job drops its `go install ...@v2.22.4` step and the `govulncheck` job drops the third-party `golang/govulncheck-action` (which `go install ...@latest` — unpinned); both now run via `go tool -modfile=tools/go.mod <name> ...` (`govulncheck -format sarif` keeps the existing Code Scanning upload). A **separate** module is used deliberately so the tools' heavy transitive dependencies (gRPC, OpenTelemetry, the Google Cloud SDK, …) stay out of the root `go.mod`/`go.sum` and are never inherited by consumers importing `go-glx` as a library. New `make gosec`, `make vulncheck`, and `make security` targets run the same pinned tools locally; a `tools-tidy-check` target (and matching `validate-spec` CI step) keeps `tools/go.mod` tidy, and Dependabot's `gomod` ecosystem now watches `/tools` so the pins stay current. `golangci-lint` (pinned via `.golangci-lint-version`, see #272) and `goreleaser` intentionally remain on their dedicated actions. (#480)
