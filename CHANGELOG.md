...
- **`civil_union` and `common_law_marriage` standard relationship types** — Both are legally distinct from marriage in many jurisdictions and exist as separate categories in GEDCOM X (`CivilUnion`, `CommonLawMarriage` couple facts) and Gramps (`CIVIL_UNION`), so imports from software that distinguishes them no longer have to collapse the distinction into `marriage` or `partner`. Added to `relationship-types.glx` with parallel `RelationshipType*` constants in `go-glx/constants.go`. Neither has a dedicated GEDCOM 5.5.1/7.0 tag (both round-trip as `MARR` with a free-text `TYPE`, preserved via the existing `marriage_type` property — documented in each entry's comment). The `partner` description gains a sentence delimiting it from both new types: `partner` stays the home for informal domestic partnership/cohabitation, while `civil_union` covers the legally formalized kind. Gramps' `UNMARRIED` is deliberately not added as a type — an unmarried couple is `partner` by definition. (#556)
