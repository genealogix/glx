...
- **Release workflow hardening** — Release runs are serialized through one concurrency group with `cancel-in-progress: false`, so a hotfix tag pushed behind a still-running release queues rather than racing its asset uploads, cosign signing, and Discord announce; cancelling is deliberately avoided since killing a run between upload and signing would leave an unsigned release (#1008). The GoReleaser tool version is pinned to a patch-floating minor (`~> v2.18`) — the action SHA pin covers the action, not the tool binary it downloads, so `~> v2` could build different artifacts from the same source on a later run; the pin is invisible to Dependabot and needs a currency gate (#1215) (#1009). The Discord announce payload is built with `jq --arg` instead of hand-spliced `\"` escapes, since the `-beta*` trigger glob admits any non-slash character and git permits `"` in a ref name, so a raw tag splice could emit malformed JSON; the tag is `@uri`-encoded where it lands in a URL, and `${{ github.repository }}` in `run:` is replaced by the runner-provided `$GITHUB_REPOSITORY` per the repo's env-staging rule (#1010).
