...
- **`/check-docs-drift` slash command invokes the real validator on extracted YAML snippets** — Step 2 ("Example Code Blocks in Documentation") of `.claude/commands/check-docs-drift.md` previously asked the LLM to *mentally simulate* JSON-schema validation of YAML examples embedded in docs. It now extracts each snippet to a temp file (`mktemp -d /tmp/glx-drift-XXXXXX`) and runs `./bin/glx validate` on it, matching the deterministic pattern already used by `/check-examples`. The prompt's `allowed-tools` frontmatter gains `Bash(mktemp:*)`, `Bash(./bin/glx validate:*)`, and `Bash(rm -rf /tmp/glx-drift-*:*)`; `.claude/settings.json` `permissions.allow` gains the matching `mktemp` and `rm` entries. Failing snippets are flagged **CRITICAL**. Closes #832.
