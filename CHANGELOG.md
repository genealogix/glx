...
- **`participant-roles.glx` clarified `principal` and `subject` as synonyms** — The previous wording described `subject` as "preferred over 'principal'", but every code path (`glx init`, GEDCOM import, census import, all tests) emits `principal`. The two role descriptions now state the actual contract: `principal` is canonical, `subject` is an accepted synonym. No data migration required; both terms continue to be accepted by tooling. Fixes #523.
