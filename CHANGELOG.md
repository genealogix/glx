...
- **`/check-docs-drift` slash command invokes the real validator on extracted YAML snippets** — Step 2 ("Example Code Blocks in Documentation") of `.claude/commands/check-docs-drift.md` previously asked the LLM to *mentally simulate* JSON-schema validation of YAML examples embedded in docs. It now extracts each snippet to a temp file (`mktemp -d /tmp/glx-drift-XXXXXX`), writes the body via a single-quoted heredoc (so YAML containing `$`, backticks, or quotes is preserved literally), runs `./bin/glx validate` on it, and cleans up per-snippet — matching the deterministic pattern already used by `/check-examples`. The prompt's `allowed-tools` frontmatter gains `Bash(mktemp -d /tmp/glx-drift-*:*)`, `Bash(cat:*)`, `Bash(./bin/glx validate:*)`, and `Bash(rm -rf /tmp/glx-drift-*:*)` (all narrowly scoped to the slash command's actual call sites); `.claude/settings.json` `permissions.allow` gains the matching `mktemp`, `cat`, and `rm` entries. Failing snippets are classified by exit code AND the snippet's own top-level YAML keys: recognized or plausibly-typoed archive wrappers (`persons:`, `people:`, etc.) → **CRITICAL** drift; entity-property / vocabulary-entry / fragment shape → `category: snippet_not_archive_shape` informational, pending Phase 2's `--stdin --entity-type` flag for in-place fragment validation. Closes #832.
