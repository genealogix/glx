...
- **`glx link --create-source` and `glx add` entity-ID slugs now transliterate diacritics instead of dropping them** — `slugifyForID` (shared by `glx link --create-source` and every `glx add` subcommand: person, place, event, source, repository, relationship, assertion) and the census `slugifyString` previously lowercased the title/name and replaced every run of non-`[a-z0-9]` with a hyphen, so non-ASCII letters were silently dropped, leaving stray hyphens that split words mid-token (`ausgewählte` → `ausgew-hlte`, `Kirchenbücher` → `kirchenb-cher`, `Saint-Étienne` → `saint-tienne`). A new shared `TransliterateForSlug` helper in `go-glx` now runs first: German umlauts and ß expand to their conventional digraphs (`ä→ae`, `ö→oe`, `ü→ue`, `ß→ss`), and all other accented Latin letters are reduced to their base form via Unicode NFKD normalization with combining marks stripped (`é→e`, `å→a`, `ñ→n`, `ç→c`). So `--create-source "Deutschland, ausgewählte evangelische Kirchenbücher …"` now yields `source-deutschland-ausgewaehlte-evangelische-kirchenbuecher-…` instead of `source-deutschland-ausgew-hlte-evangelische-kirchenb-cher-…`. Closes #896
