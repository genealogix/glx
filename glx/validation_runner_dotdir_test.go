...
func TestValidateSingleFileSemantics_SkipsDotDirectories(t *testing.T) {
	dir := t.TempDir()
	broken := filepath.Join(dir, ".git", "broken.glx")
	require.NoError(t, os.MkdirAll(filepath.Dir(broken), 0o755))
	require.NoError(t, os.WriteFile(broken, []byte("not: [valid"), 0o644))

	errs, warns := validateSingleFileSemantics([]string{dir})
