...
// warning is emitted so the user can reconcile by hand. Only person-subject
// assertions are touched — a non-person subject (event, relationship) may
// legitimately carry a custom `ssn` property unrelated to this migration.
func migrateSsnToNationalID(archive *glxlib.GLXFile, warnOut io.Writer) *MigrateReport {
	...
	report.SsnPropertiesRenamed = renamePersonSsnProperties(archive, warnOut)
	report.SsnAssertionsRenamed = renameSsnAssertions(archive)
...
// A person already carrying `national_id` is skipped with a warning rather than
// overwritten. Persons are visited in sorted order so warnings are
// deterministic.
func renamePersonSsnProperties(archive *glxlib.GLXFile, warnOut io.Writer) int {
	count := 0
		...
		if _, hasNationalID := person.Properties[nationalIDProperty]; hasNationalID {
			_, _ = fmt.Fprintf(warnOut,
				"Warning: person %s carries both legacy `ssn` and `national_id` "+
					"properties — leaving `ssn` in place to avoid overwriting "+
					"`national_id`. Reconcile manually.\n", personID)
...
func renameSsnAssertions(archive *glxlib.GLXFile) int {
	count := 0
