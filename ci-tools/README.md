...
- **gosec** stays on a version-pinned `go install` in
  ...
  without exposing that tree. Run it locally with `make gosec`. Dependabot cannot
  propose a bump for this pin: its `gomod` ecosystem only reads `go.mod` files,
  where gosec deliberately does not appear, and its `github-actions` ecosystem
  only reads `uses:` references, not a `go install ...@vX.Y.Z` line in a `run:`
  block. So the weekly `.github/workflows/gosec-pin-currency.yml` workflow fails
  when `.gosec-version` falls behind the latest gosec release. It is a standalone
