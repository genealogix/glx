...
Use `confidence` to express certainty and `status` to track research state (`proven`, `speculative`, `disputed`, `disproven`). These are independent — `confidence: high` + `status: speculative` means "I'm fairly sure, but haven't verified yet." See [Assertion Entity](/specification/4-entity-types/assertion#status).
