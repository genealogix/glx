...
func (glx *GLXFile) validateDateFormat(entityType EntityType, entityID, field, dateStr string, result *ValidationResult) {
	...
	if _, err := glxdate.Parse(dateStr); err != nil {
		result.Warnings = append(result.Warnings, ValidationWarning{
			...
			SourceID:   entityID,
			Field:      field,
			Message: fmt.Sprintf("%s[%s].%s: date '%s' should be in format YYYY, YYYY-MM, or YYYY-MM-DD, optionally with a keyword (ABT, BEF, AFT, CAL, EST, INT, BET … AND, FROM … TO), a calendar prefix (JULIAN, HEBREW, FRENCH_R, or an extension calendar such as _ROMAN), or a BCE suffix: %s",
...
func (glx *GLXFile) validateEntityFieldFormats(result *ValidationResult) {
	...
	for id, study := range glx.Studies {
		if study != nil && study.DateRange != "" {
			glx.validateDateFormat(EntityTypeStudies, id, "date_range", string(study.DateRange), result)
		}
	}
