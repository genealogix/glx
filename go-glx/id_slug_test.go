...
func TestSlugify(t *testing.T) {
	tests := []struct {
		...
		// Decomposed (NFD) inputs written with the combining diaeresis as
		// an explicit Go Unicode escape for codepoint U+0308 on the base
		// letter, rather than as a literal codepoint, so an editor or save
		// round-trip cannot silently re-normalize the source to the
		// precomposed form and defeat the test. Slugify must compose to NFC
		// first so these reach germanSlugReplacer in precomposed form and
		// expand to the digraph; otherwise the umlaut falls through to NFKD
		// + mark-strip and is silently reduced to a bare vowel ("u" instead
		// of "ue").
