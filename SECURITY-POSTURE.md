...
| Direct commits to primary branch prevented | ✓ | Active **Main Protection** ruleset on the default branch enforces `pull_request`, `required_status_checks`, `non_fast_forward`, and `deletion` rules |
...
| Automated test suite runs before merge | ✓ | The Main Protection ruleset requires the **Validate Specification**, **Lint**, **Security**, **Lint Markdown**, **Dependency Review**, and **Lint PR Title** workflows to pass on every PR |
...
GLX is a local CLI and Go library. There is no service, no telemetry, no auto-update channel, no network listener in normal operation. On its own, GLX is not a "product with digital elements" within the meaning of the CRA.
