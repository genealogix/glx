...
> As of **2026-09-17**, GLX self-attests to the [OpenSSF OSPS Baseline](https://baseline.openssf.org/) version **2026.02.19** at **Level 3 (mature)**: every control tracked below is met, and no gap remains open.
...
| Continuous security posture scoring | ✓ | [OpenSSF Scorecard](https://github.com/genealogix/glx/blob/main/.github/workflows/scorecard.yml) runs on push to `main`, weekly on Mondays, and on `workflow_dispatch`. Results are published to the public OSSF dataset and uploaded to GitHub Code Scanning as SARIF. The workflow deliberately has **no** `pull_request` trigger: publishing requires OIDC (`id-token: write`), which fork PRs cannot safely hold, so per-PR Scorecard coverage would break the publish model rather than harden it |
| GitHub Actions pinned per a documented convention | ✓ | Third-party actions (including `github/*`) are pinned to full commit SHAs with a trailing version comment; one action that publishes no floating major tag (`sigstore/cosign-installer`) is pinned to an exact patch tag instead. First-party `actions/*` deliberately use floating major tags, with `actions/attest` as the single exception. See [Action pinning](#action-pinning) for the rationale and the accepted Scorecard consequence |
...
| [`mszostok/codeowners-validator`](https://github.com/mszostok/codeowners-validator) | Full commit SHA plus version comment | [`codeowners.yml`](https://github.com/genealogix/glx/blob/main/.github/workflows/codeowners.yml) |
...
The exact-patch-tag form is the weakest of the three — a patch tag is still a mutable ref — and is used only where a SHA pin has not been adopted. None of them may be "corrected" to a floating `@vN`, which fails outright with `Unable to resolve action`.
...
Every release publishes one SPDX-JSON SBOM per archive, named after the archive it describes — for example `glx_Linux_x86_64.tar.gz.sbom.json` accompanies `glx_Linux_x86_64.tar.gz`. Because the SBOMs are listed in `checksums.txt`, verifying one is the same two-step flow as verifying an archive: check the manifest's signature, then check the file against the manifest.
...
None. Every Level 1, Level 2, and Level 3 control tracked above is met as of 2026-09-17, when SBOM emission ([#269](https://github.com/genealogix/glx/issues/269)) closed the last gap.
