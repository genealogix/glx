...
| GitHub Actions pinned per a documented convention | ✓ | Third-party actions (including `github/*`) are pinned to full commit SHAs with a trailing `# vX.Y.Z` comment — apart from actions that publish no floating major tag, which are pinned to an exact patch tag. First-party `actions/*` deliberately use floating major tags, with privileged jobs excepted. See [Action pinning](#action-pinning) for the rationale and the accepted Scorecard consequence |

#### Action pinning

Third-party GitHub Actions used by this repository are pinned to a full commit SHA with a trailing `# vX.Y.Z` version comment, which is the form the OpenSSF Scorecard *Pinned-Dependencies* check expects. Actions published under the `github` organization (for example `github/codeql-action`) are treated as third-party for this purpose and are SHA-pinned like any other. The one variation is an action that publishes no floating major tag at all (today `sigstore/cosign-installer`): it is pinned to an exact patch tag, which is immutable in practice but is not a commit SHA.

Actions published under the `actions` organization — `checkout`, `setup-go`, `setup-node`, `upload-artifact`, `download-artifact`, `cache`, and friends — deliberately use floating major tags (`@v7`, `@v8`, and so on, whatever the action's current major is). These actions are maintained by GitHub and execute on GitHub-hosted runners, inside the same trust boundary as the runner itself, so a SHA pin bounds little additional risk. Pinning them would also suppress Dependabot vulnerability *alerts*, which GitHub raises only for actions referenced by semantic-version tag and [never for SHA-pinned references](https://docs.github.com/en/actions/reference/security/secure-use). The references are kept current by the weekly `github-actions` Dependabot ecosystem entry in [`.github/dependabot.yml`](https://github.com/genealogix/glx/blob/main/.github/dependabot.yml).

The exception is privileged jobs: `actions/attest` in [`release.yml`](https://github.com/genealogix/glx/blob/main/.github/workflows/release.yml) is SHA-pinned, because it runs in the release job that holds `id-token: write`, where a re-pointed tag would sit upstream of artifact signing. Any future privileged job is pinned the same way.
